...
namespace MARTe
{
    ...
    bool MDSParameter::Actualize(ConfigurationDatabase &targetcdb, MDSClientTypes &clienttype, MDSplus::Connection *conn,
                                 MDSplus::Tree *treeName, StreamString &baseClassName)
    {
        ...
        {
            // Adds indices to the path in the form "[startIdx:stopIdx]"
    ...
    {
        bool status = ObjParameter::Initialise(data);

        if (status)
        {
            if (!data.Read("Path", MDSPath))
            {
                status = false;
        ...
        if (status)
        {
            if (data.Read("DataOrientation", MDSOrientation))
            {
        ...
        if (status)
        {
            if (data.Read("StartIdx", startIdx))
            {
    ...
    bool MDSParameter::TransposeAndCopy(TypeDescriptor type, void *const destination, const void *const source)
    {
        ...
        {
            REPORT_ERROR(ErrorManagement::Exception, "Unsupported type.");
        }
