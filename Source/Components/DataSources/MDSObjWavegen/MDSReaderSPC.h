/**
 * @file MDSReaderSPC.h
 * @brief Header file for class MDSReaderSPC
 * @date 22/09/2017
 ...
 * @details This header file contains the declaration of the class MDSReaderSPC
...
/**
 * @brief MDSReaderSPC is a data source which allows to read data from a MDSplus tree.
 * @details MDSReaderSPC is an input data source which takes data from MDSPlus nodes (as many as desired) and publishes it on a real time application.
 *
 * The MDSReaderSPC can either interpolate, decimate or take the raw data, as it is, from the tree depending on the parameter called "DataManagement" which is given
 ...
 * <ul>
 * <li>0 --> MDSReaderSPC takes the data from the tree as it is (raw). In this configuration, the frequency/numberOfElements must be the same than the node sampling frequency.</li>
 * <li>1--> MDSReaderSPC interpolates the signal taking as a reference the two nearest data values. If the frequency/numberOfElements is smaller than the sample frequency
 ...
 * <li>2 --> MDSReaderSPC holds the value following the criteria of the nearest value given specific time. I.e the node data is (t1, d1) = (1, 1) and (t2, d2) = (2, 5) and the currentTime is t = 1.6 the
 ...
 * <li>1 --> MDSReaderSPC fills the absence of data with the last value.</li>
 * </ul>
 *
 * Even if the MDSReaderSPC can deal with the absence of data, the sampling time must be constant with-in the node, however the sampling time between
 * nodes can be different.
 *
 * The MDSReaderSPC can handle as many nodes as desired. Each node can have their on data type, maximum number of segments, elements per segment and sampling time. When the
 ...
 * </pre>
 */
class MDSReaderSPC: public DataSourceI {
//TODO Add the macro DLL_API to the class declaration (i.e. class DLL_API MDSReaderSPC)
public:
...
private:
     ...
     * @details this function decides the type of data to copy and then calls the MDSReaderSPC::CopyTheSameValue()
...
/*---------------------------------------------------------------------------*/
template<typename T>
void MDSReaderSPC::CopyTheSameValueTemplate(uint32 idxNumber,
                                         uint32 numberOfTimes,
                                         uint32 samplesOffset) {
...
template<typename T>
uint32 MDSReaderSPC::MakeRawCopyTemplate(uint32 nodeNumber,
                                      uint32 minSeg,
                                      uint32 SamplesToCopy,
                                      uint32 OffsetSamples) {
...
template<typename T>
uint32 MDSReaderSPC::LinearInterpolationCopyTemplate(uint32 nodeNumber,
                                                  uint32 minSeg,
                                                  uint32 samplesToCopy,
                                                  uint32 offsetSamples) {
...
template<typename T>
uint32 MDSReaderSPC::HoldCopyTemplate(uint32 nodeNumber,
                                   uint32 minSeg,
                                   uint32 samplesToCopy,
                                   uint32 samplesOffset) {
...
template<typename T>
bool MDSReaderSPC::SampleInterpolation(float64 cT,
                                    T data1,
                                    T data2,
                                    float64 t1,
                                    float64 t2,
