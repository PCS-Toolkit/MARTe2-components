/**
 * @file MDSReaderSPC.cpp
 * @brief Source file for class MDSReaderSPC
 * @date 22/09/2017
 ...
 * the class MDSReaderSPC (public, protected, and private). Be aware that some
...
bool MDSReaderSPC::Initialise(StructuredDataI& data) {
    ...
    /// GOES, the originalSignalInformation cdb will be build by the SCD wavegen classes
    if (ok) {
...
bool MDSReaderSPC::SetConfiguredDatabase(StructuredDataI & data) {
    bool ok = DataSourceI::SetConfiguredDatabase(data);
    if (!ok) {
        REPORT_ERROR(ErrorManagement::ParametersError, "DataSourceI::SetConfiguredDatabase(data) returned false");
    }

    /// DO NOT KNOW, Signals shuold be handled by the derived SCD MDS classes
    if (ok) {
        ok = data.MoveRelative("Signals");
    }

    /// DO NOT KNOW, asked Andre'
    if (ok) { // Check that only one GAM is Connected to the MDSReaderSPC
    ...
    /// GOES, UNLESS TIME SIGNAL IS KEPT
    ...
    /// GOES, numberOfNodeNames MUST BE SET BY SCD WG CLASSES
    if (ok) {
    ...
    /// THIS CHECK MUST BE MOVED TO SCD MDS WG
    if (ok) {
    ...
    /// STAY, WITH originalSignalInformation FILLED BY THE SCD WG CLASSES
    ...
    /// GOES, GetSignalType WOULD ACT ON THE WRONG SIGNAL CDB
    ...
    /// STAYS IF WE KEEP THE TIME INPUT
    ...
    /// DON'T KNOW , PERHAPS WE HAVE TO CHECK IF THE SIGNALS ARE SCALAR
    ...
    /// GOES, THE NUMBER OF ELEMENTS IS FIXED BY
...
bool MDSReaderSPC::AllNodesEnd() const {
    bool ret = true;
    if (endNode != NULL_PTR(bool *)) {
...
bool MDSReaderSPC::PrepareNextState(const char8 * const currentStateName,
                                 const char8 * const nextStateName) {
    return true;
}

bool MDSReaderSPC::AllocateMemory() {
    return true;
}

uint32 MDSReaderSPC::GetNumberOfMemoryBuffers() {
    return 1u;
}

/*lint -e{715}  [MISRA C++ Rule 0-1-11], [MISRA C++ Rule 0-1-12]. Justification: The signalAddress is independent of the bufferIdx.*/
bool MDSReaderSPC::GetSignalMemoryBuffer(const uint32 signalIdx,
...
const char8 *MDSReaderSPC::GetBrokerName(StructuredDataI &data,
...
bool MDSReaderSPC::GetInputBrokers(ReferenceContainer &inputBrokers,
...
bool MDSReaderSPC::GetOutputBrokers(ReferenceContainer &outputBrokers,
                                 const char8* const functionName,
                                 void * const gamMemPtr) {
    return false;
}

bool MDSReaderSPC::OpenTree() {
    bool ret = true;
    try {
...
bool MDSReaderSPC::OpenNode(const uint32 idx) {
    bool ret = true;
...
bool MDSReaderSPC::GetTypeNode(const uint32 idx) {
    bool ret = true;
    if (nodes != NULL_PTR(MDSplus::TreeNode **)) {
...
bool MDSReaderSPC::IsValidTypeNode(const uint32 idx) const {
    bool ret = true;
    if (mdsNodeTypes != NULL_PTR(StreamString *)) {
        ...
        else if (mdsNodeTypes[idx] == "DTYPE_SIGNAL") {
...
bool MDSReaderSPC::CheckTypeAgainstMdsNodeTypes(const uint32 idx) const {
    bool ret = true;
...
TypeDescriptor MDSReaderSPC::ConvertMDStypeToMARTeType(StreamString mdsType) const {
    TypeDescriptor str;
    if (mdsType == "DTYPE_BU") {
...
bool MDSReaderSPC::GetDataNode(const uint32 nodeNumber) {
    bool ok;
...
//lint -e{613} Possible use of null pointer. Not possible. If initialise fails this function is not called.
int8 MDSReaderSPC::FindSegment(const float64 t,
                            uint32 &segment,
...
uint32 MDSReaderSPC::CheckDiscontinuityOfTheSegments(const uint32 nodeNumber,
...
bool MDSReaderSPC::GetNodeSamplingTime(const uint32 idx,
                                    float64 &tDiff) const {
    bool ret = true;
...
void MDSReaderSPC::CopyTheSameValue(const uint32 idxNumber,
...
bool MDSReaderSPC::AddValuesCopyData(const uint32 nodeNumber,
                                  uint32 minSegment,
...
//lint -e{613} Possible use of null pointer. Not possible. If initialise fails this function is not called.
uint32 MDSReaderSPC::ComputeSamplesToCopy(const uint32 nodeNumber,
...
void MDSReaderSPC::VerifySamples(const uint32 nodeNumber,
                              uint32 &samples,
                              const float64 tstart,
                              const float64 tend) const {
...
//lint -e{613} Possible use of null pointer. If initialisation fails this function is not called.
bool MDSReaderSPC::CopyDataAddValues(const uint32 nodeNumber,
                                  uint32 minSegment,
                                  const uint32 numberOfDiscontinuities,
...
//lint -e{613} Possible use of null pointer. If initialisation fails this function is not called.
bool MDSReaderSPC::AddValuesCopyDataAddValues(const uint32 nodeNumber,
...
bool MDSReaderSPC::CopyDataAddValuesCopyData(const uint32 nodeNumber,
                                          uint32 minSegment,
                                          const uint32 numberOfDiscontinuities) {
    bool ret = true;
    uint32 numberOfSamplesToCopy = 0u;
...
//lint -e{613} Possible use of null pointer. Not possible.If initialisation fails this function is not called
bool MDSReaderSPC::FindDiscontinuity(const uint32 nodeNumber,
                                 uint32 &segment,
...
uint32 MDSReaderSPC::MakeRawCopy(const uint32 nodeNumber,
                              const uint32 minSeg,
...
uint32 MDSReaderSPC::LinearInterpolationCopy(const uint32 nodeNumber,
                                          const uint32 minSeg,
                                          const uint32 samplesToCopy,
                                          const uint32 offsetSamples) {
...
uint32 MDSReaderSPC::HoldCopy(const uint32 nodeNumber,
                           const uint32 minSeg,
                           const uint32 samplesToCopy,
                           const uint32 samplesOffset) {
...
bool MDSReaderSPC::CopyRemainingData(const uint32 nodeNumber,
                                  const uint32 minSegment) {
    bool ret = false;
...
CLASS_REGISTER(MDSReaderSPC, "1.0")
}
